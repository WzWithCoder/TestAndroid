...
class Statusbar_KitKat {
    private static final String TAG_FAKE_STATUS_BAR_VIEW = "statusBarView";
    private static final String TAG_MARGIN_ADDED = "marginAdded";

    static void setColor(Activity activity, int statusColor) {
        ...
        //获取父布局
        View contentChild = mContentView.getChildAt(0);
        //获取状态栏高度
    ...
    static void setTranslucent(Activity activity) {
        Window window = activity.getWindow();
        //设置Window为透明
        window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
    ...
    static void setColorForCollapsingToolbar(final Activity activity, final AppBarLayout appBarLayout, final CollapsingToolbarLayout collapsingToolbarLayout,
        ...
        int statusBarHeight = getStatusBarHeight(activity);
        removeFakeStatusBarViewIfExist(activity);
        removeMarginTopOfContentChild(contentChild, statusBarHeight);
        //添加一个View来作为状态栏的填充
    ...
    static void setWhiteForCollapsingToolbar(final Activity activity, AppBarLayout appBarLayout, final CollapsingToolbarLayout collapsingToolbarLayout, Toolbar toolbar, final int statusBarColor) {
            ...
            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
                if (Math.abs(verticalOffset) >= (appBarLayout.getTotalScrollRange() - StatusBar.getPxFromDp(activity, 56))) {
                    if (appBarLayoutState != COLLAPSED) {
                        ...
                        if (StatusBar.setLightModeMIUI(activity, true) || StatusBar.setLightModeFlyme(activity, true)) {
                        }
                        if (statusView.getAlpha() == 0) {
                            statusView.animate().cancel();
                            statusView.animate().alpha(1f).setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
                    ...
                    if (appBarLayoutState != EXPANDED) {
                        ...
                        if (StatusBar.setLightModeMIUI(activity, false) || StatusBar.setLightModeFlyme(activity, false)) {
                        }
                        if (statusView.getAlpha() == 1) {
                            statusView.animate().cancel();
                            statusView.animate().alpha(0f).setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
